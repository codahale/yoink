...
import com.codahale.yoink.PersistentTreeMap

class PersistentTreeMapTest extends Spec with MustMatchers {
  describe("a persistent tree map") {
    val map = PersistentTreeMap("one" -> 1, "two" -> 2, "three" -> 3)
