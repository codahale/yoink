...
import com.codahale.yoink.MutablePersistentHashMap
import org.scalatest.concurrent.Conductor

class MutablePersistentHashMapTest extends Spec with MustMatchers {
    ...
    describe("being modified concurrently") {
      val other = MutablePersistentHashMap[Int, Int]()
  ...
  def makeMap = MutablePersistentHashMap("one" -> 1, "two" -> 2, "three" -> 3)
}
